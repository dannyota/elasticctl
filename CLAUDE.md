...
`.env` holds live credentials for the stack you develop against. It is gitignored and mode
`0600`. `.env.example` is the template to copy.

- Never commit it, copy its contents into tracked files, or echo the key into terminal output or
  commit messages.
...
- Recording uses the active `ELASTICCTL_*` environment variables plus
  `ELASTICCTL_FIXTURE_FLAVOR`. A git worktree has no `.env`; source it from the main checkout
...
`CLAUDE.md` is tracked by the owner's decision, carried by the one existing `.gitignore`
negation. Never force-add, track, or add negations for any other globally ignored file, and never
extend that one.
