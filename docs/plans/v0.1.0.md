...
/// Kept so an unused-import warning cannot hide a missing error path.
#[allow(dead_code)]
fn _assert_error_types_are_reachable(e: Error) -> ErrorKind {
    e.kind
}
```
...
pub async fn export(ctx: &Context, out: Option<&Path>, format: FileFormat) -> Result<Value> {
    ...
    match out {
        ...
        // Without --out the file body goes to stdout as one string, so it can
        // be piped straight into a file or another tool.
