...
- Produces:
  ...
  - `pub fn sort_rules(rules: &mut Vec<Rule>)` — by `rule_id`
...
**Files:**
- Create: `crates/elasticctl-cli/src/cli.rs`, `crates/elasticctl-cli/src/render.rs`, `crates/elasticctl-cli/src/context.rs`
...
- Produces:
  ...
  - `pub struct Context { pub resolved: Resolved, pub global: GlobalArgs }`
  - `Context::build(global: &GlobalArgs) -> Result<Context>`
...
Add to `crates/elasticctl-core/src/transport.rs`, storing `es_base` in `Transport::new` from `profile.es_url` (falling back to `kibana_url`):
...
async fn record() {
    // Reads ELASTICCTL_KIBANA_URL and ELASTICCTL_API_KEY from the environment,
    // calls each endpoint the api crate wraps, scrubs, and writes to
    // tests/fixtures/<flavor>-<version>/<operation>.json.
    //
    // Operations to record, matching the api surface:
    //   status, authenticate, rules_find_empty, rules_find_page,
    //   rules_get, rules_create, rules_patch, rules_bulk_disable,
    //   rules_export, rules_import, rules_preview, rules_delete
    //
    // Each fixture body: {"flavor":..,"version":..,"request":{..},"response":{..}}
    todo_record_operations().await;
}

async fn seed() {
    // Installs prebuilt Elastic rules through
    // POST /api/detection_engine/rules/prepackaged so pull, diff, and preview
    // have real data to work against.
    todo_seed_prebuilt().await;
}
```

**Note for the implementer:** the two `todo_` calls above are the task's actual work — write them out against the endpoint list given in the comments, using `elasticctl_core::Transport`. They are named rather than left blank so a missing implementation fails to compile instead of silently recording nothing.
