...
cargo add --package elasticctl-cli serde serde_yaml_ng --features serde/derive
# serde_json is inherited: add `serde_json.workspace = true` to each crate manifest
cargo add --package elasticctl-cli --dev assert_cmd predicates tempfile
```
...
**Why the guard owns the declaration.** `mutates` in the command tree (Task 18)
is what a future MCP server reads to decide whether a tool call needs
confirmation, and it cannot be inferred from clap metadata — nothing in a
`clap::Command` says "this writes". The guard is the one chokepoint every
remote mutation already passes through, so making it assert its caller is
declared converts "someone forgot to update a list" from a silent wrong answer
into a test failure. Declaring `MUTATING` here rather than in `meta.rs` keeps
the constant next to the code that enforces it.

A caller that shares one helper between two commands must derive the path from
the same value that chooses the behaviour — `set_enabled` picks
`"rules enable"` or `"rules disable"` from its `enabled` flag. A hardcoded
string there would make the assert pass while lying, which is worse than no
assert at all.
...
**Directory layout.** One file per rule under `<dir>/rules/`, named `<rule_id>.<ext>`. One file per rule rather than one big file because it gives git per-rule history and blame, which is the whole reason for keeping rules in version control. Rule ids are used verbatim as filenames after sanitising path separators. Sanitising is lossy, so two distinct rule ids can land on one filename (`"a/b"` and `"a_b"` both become `a_b.ndjson`). `pull` must detect that **before** the second write and fail with `ErrorKind::Conflict` naming both ids and the shared filename — silently overwriting drops a rule from the mirror while the pulled count still counts both.
...
A failure on one rule does not abort the rest. Each entry records its own `applied` flag and error, so a partial push is fully auditable. A push with any failure exits 1 **after** rendering the report — the same partial-failure convention as Task 14.

The report records what was *proposed*, not only what was applied. Every actionable change gets an entry in both modes; on a dry run the entry carries `applied: false`, `before` = remote state (null for a create), `after` = the desired local rule, and the JSON summary carries a `pending` count. Without this a script piping `state push --json` sees `created: 0, updated: 0` and cannot tell "nothing to do" from "everything is waiting on `--yes`".

`read_local` scans only recognised rule extensions (`ndjson`, `json`, `yaml`, `yml`) and skips everything else silently. One file per rule means a `README.md` or a stray `.DS_Store` in `rules/` is expected, and it must not hard-fail every `diff` and `push`. Do not widen `Format::from_path` to achieve this — its NDJSON default is correct where the user named the file deliberately (`--out`, `--path`).
...
back from `MUTATING` — `let expected = MUTATING;` — makes the assertion
`MUTATING == MUTATING`, which passes for any value including a wrong one. The
duplication is the point: the test is a second, hand-written statement of the
contract, and the two disagreeing is the signal.

**Do not re-derive `MUTATING` here.** It lives in `guard.rs` (Task 11) because
the guard is what enforces it: `guard::check` asserts its caller is declared,
so an undeclared mutation fails its own tests. Two copies would drift, and the
copy in this file is the one nothing checks.

**What this tree does and does not guarantee.** The exhaustive test below pins
the declared set so it cannot silently grow. The guard assert catches an
undeclared command that routes through the guard. Nothing catches a mutating
command that neither calls the guard nor is declared — clap metadata carries no
independent signal of mutation, so do not write a comment claiming otherwise.
...
**Two settings that cost an afternoon if missed.** Kibana needs its encrypted-saved-objects key set to 32 or more characters, or **every rule creation fails** with an error that never mentions encryption. In the compose file that key must be the `XPACK_ENCRYPTEDSAVEDOBJECTS_ENCRYPTIONKEY` environment variable — the docker entrypoint only maps the UPPER_SNAKE form of a setting to a `--longopt`, and the dotted `xpack.encryptedSavedObjects.encryptionKey` form is silently ignored, so Kibana starts with a random key. The detection engine also needs its signals index bootstrapped through `POST /api/detection_engine/index` before rules will run.
...
`lab/seed.sh` installs the prebuilt rules against the running lab. It exists
because the recorder reads its connection from `ELASTICCTL_KIBANA_URL`,
`ELASTICCTL_ES_URL`, and `ELASTICCTL_API_KEY` — not a config profile — so it
mints a key and exports those three variables for `cargo xtask seed`:
...
`xtask/src/main.rs` records real exchanges and scrubs both credentials and the
operator's identity before writing. Fixtures are named
`<flavor>-<version>/<operation>.json` and each carries the flavor and stack
version it came from, so drift is visible. Two properties of `scrub` must not
be simplified away: it matches identity keys (`username`, `full_name`,
`email`, `created_by`, `updated_by`) as well as credential keys, and it redacts
a value without changing its type — a fixture pins a response's *shape*, so an
object `api_key` must stay an object, and a null `full_name` must stay null.
...
`ELASTICCTL_LIVE=1`, so `cargo test` stays offline by default. Three parts are
load-bearing and must not be stubbed back out: the `LIVE_LOCK` mutex serializes
the round-trip test (which mutates the shared remote) against the pull/diff
test (which asserts the remote is stable across two reads); the `Drop`-based
`Cleanup` guard deletes the probe rule on every exit path and retries, so a
failed assertion cannot strand a rule for the next run; and the generated
config is written `0600`, because the tool flags a world-readable config as
insecure.
...
`crates/elasticctl-api/tests/fixtures.rs`. The recorder writes fixtures, but
nothing before this step reads them, so a fixture that stops matching the live
client after an Elastic upgrade would drift unnoticed. This test consumes every
recorded set through the same decode paths the live client uses, so a re-record
that changes a response shape fails here. Asserting that a file merely parses
is not enough — the assertions must reach decoded values, not just check that
...
Add a second `live` job to `.github/workflows/ci.yml` that runs on a schedule
and on manual dispatch, standing up the lab and running the live suite. The
default `test` job stays offline. The `timeout-minutes: 45` cap is the outer
bound that lets the teardown step run even if the lab misbehaves:
...
Expected: fixtures under `tests/fixtures/traditional-9.5.1/`. This is the
session that keeps 0.1.0 from being a serverless-only tool wearing a
three-flavor label. Do not skip it.
