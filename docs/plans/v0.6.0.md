...
- `get` returns only `{id, name, namespace, description, agents, status, attached_integrations, blocked_by}`. It never returns raw audit fields, environment ids, the agentless configuration, or populated integration objects. A truncated CLI list emits `capped at <limit> rows` on stderr.
...
- Produces `PackageStatus { pub name: String, pub status: String, pub installed_version: Option<String>, pub latest_version: Option<String> }`.
...
async fn package_status_reads_only_the_installation_facts() {
    ...
    assert_eq!(
        ...
        PackageStatus {
            ...
            latest_version: Some("2.5.0".into()),
        }
    );
}
...
pub async fn package_status(transport: &Transport, name: &str) -> Result<PackageStatus> {
    ...
    Ok(PackageStatus {
        ...
        status,
        installed_version,
        latest_version: optional_non_empty_string(item, "latestVersion")?,
    })
}
...
#[tokio::test]
async fn get_rejects_missing_agent_or_attachment_facts() {
    for missing in ["agents", "package_policies"] {
        let server = verified_server().await;
        ...
        assert_eq!(error.kind, ErrorKind::Http, "{missing}: {}", error.message);
    }
}
...
fn required_agents(item: &Map<String, Value>, id: &str) -> Result<u64> {
    item.get("agents")
        ...
        .ok_or_else(|| http(format!("decoding agent policy '{id}': expected unsigned agents")))
}
...
fn replace_body_strips_id_nulls_clearable_fields_and_refuses_other_removals() {
    ...
    let error = agent_policy_ops::build_replace_body(&nested_current, &nested_desired).unwrap_err();
    assert_eq!(
        error.message,
        "removing advanced_settings.agent_logging_level is not supported by the agent-policy update API"
    );
}
...
pub fn build_replace_body(current: &AgentPolicySpec, desired: &AgentPolicySpec) -> Result<Value> {
    ...
    for (field, before, after) in [
        ("advanced_settings", &current.advanced_settings, &desired.advanced_settings),
        ("overrides", &current.overrides, &desired.overrides),
        ("monitoring_http", &current.monitoring_http, &desired.monitoring_http),
        ("monitoring_diagnostics", &current.monitoring_diagnostics, &desired.monitoring_diagnostics),
    ] {
        if let (Some(before), Some(after)) = (before, after)
            && let Some(path) = first_removed_key(before, after, field)
        {
            return unsupported(format!(
                "removing {path} is not supported by the agent-policy update API"
            ));
        }
    }
...
/// Depth-first: the first key present in `before` and absent in `after`.
fn first_removed_key(before: &Map<String, Value>, after: &Map<String, Value>, prefix: &str) -> Option<String> {
        ...
        match after.get(key) {
            ...
            Some(Value::Object(after_object)) => {
                if let Value::Object(before_object) = before_value
                    && let Some(nested) = first_removed_key(before_object, after_object, &path)
                {
                    return Some(nested);
                }
            }
...
- A create whose mutation and stored-spec check succeed while the post-write
  package read still reports `not_installed` must produce one failed row with
  `applied: true`; `package_installs` stays empty. A package read error must
  produce the same applied failure instead of being swallowed.
        ...
        match (before, current) {
            ...
            (before, current) => {
                ...
                let package_error = if package_can_change {
                    ...
                    match verify_package_after_write(transport, expected, applied).await {
                        Ok((after, installed)) => {
...
async fn verify_package_after_write(
    transport: &Transport,
    before: &agent_policies::PackageStatus,
    policy_write_succeeded: bool,
    ...
    if before.status == "installed" {
        return if after == *before {
            ...
            Err("elastic_agent package changed during apply".into())
        };
    }
    if after.status == "installed" {
        let version = after.installed_version.as_ref().expect("decoder requires installed version");
        return Ok((after.clone(), Some(format!("{MONITORING_PACKAGE}@{version}"))));
    }
    if policy_write_succeeded {
        Err("elastic_agent was not installed after the policy write".into())
    } else if after == *before {
        Ok((after, None))
    } else {
        Err("elastic_agent package changed during failed policy write".into())
    }
}
...
- Modify: `crates/elasticctl-cli/src/guard.rs:158-186`
...
async fn agent_policy_fixtures_decode_through_the_production_paths() {
    use elasticctl_api::fleet::{agent_policies, agent_policy_ops};
