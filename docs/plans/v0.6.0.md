...
Findings the 0.6.0 task and whole-branch reviews judged non-blocking. Each is small; none changes a contract.

- `resolve` re-reads a policy it already fetched by id, so `get`, `export`, and `delete` issue two GETs per id selector. Return the fetched item from `resolve` before 0.6.1 copies the shape.
- `cmd::fleet::import` validates the artifact and `plan_import` reads it again; the "needs at least one agent policy" message lives in both crates.
- `ExportOutcome.missing` is always empty for agent policies because every selector must resolve; add a comment at the population site.
- The recorder's create request omits `agent_features` and `global_data_tags`, which production always sends; the measured fact is unaffected.
- Tests to add: the empty active-space fallback in `portability_reasons`; a selector that is both an id and another policy's name; the serialized key order of the import report; a `body_json` matcher on the apply-time PUT; the empty-to-non-empty monitoring replace install path; a replace-row variant of the tamper test that asserts the reason; the empty-selector guard and id-plus-own-name dedup for delete; `query_param_is_missing("force")` on the delete POST mock; `export --out`, `validate`'s `{valid, total}`, and a non-empty `failed` exiting 1 in the fleet CLI tests.
- `is_platform_owned` duplicates the flag loop of `portability_reasons` over a narrower set (documented and intentional).
- `import_details` has an unreachable arm that drops a spec without a snapshot; make it fail loudly or assert.
- A live stack holding two policies with the same name fails the whole import even when no imported name is involved; scoping needs a spec sentence.
- `affected_agents` on the delete report is a literal zero, equal by construction; sum the snapshot counts.
- `validate_delete_plan` collapses every tamper cause into one message and lets `spec.validate()` leak a different one; adopt import's `invalid(<reason>)` closure.
- `plan_delete` aborts at the first `read_live` error while agent and integration conflicts aggregate across ids.
- Bare `export` reaches `require_credential` before the local selector error; `required_unless_present` on the clap arg makes it a pure argument error. `get` and `export` selectors lack `value_parser = non_empty`.
- `cmd::fleet::list` uses `filter.limit.expect(..)` on a truncated list; an `if let` removes a cross-crate invariant panic.
- The dry-run write assertion in `tests/fleet.rs` checks POST only; assert no POST, PUT, DELETE, or PATCH.
- `verified_server` and `SequenceResponder` are copied into a third CLI test file; move them to `tests/common`.
- The recorder has no preflight on `AGENT_POLICY_DUP_ID`, removes `version` instead of substituting a placeholder, and propagates raw transport errors with `?` like `record_data_views`.
- The dry-run JSON for import and delete carries no blast-radius count; the stderr preview does. An additive field if wanted.
- Spec 14's conformance package rule can install `system` on the cloud targets and keep it (user instruction, 2026-09-04); the uninstall-only-if-installed clause is unnecessary there.
- `cargo publish --workspace --dry-run --locked` fails until the release flow bumps the version past the published 0.5.2; re-run it after the bump.
- From the fix-wave re-review: `plan_import`'s echoed-id check now sits in the overwrite conversion, so a server echoing a different policy on the id read yields a skip or conflict rather than `http` in the other modes; F2's unknown-field refusal can surface post-write for a create row (`applied: true`); spec 13's "applied becomes true only after a decoded success" sentence should be narrowed to name the delete wrong-id case; the replace-row recheck tests do not assert the plan previewed `replace`; both reports set top-level `applied: true` even when every row was refused pre-write.
